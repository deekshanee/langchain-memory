...
async function runLocalExample() {
  ...
  // Start a new session
  const sessionId = memoryManager.startSession(undefined, 'Local Example Session');
  ...
  await memoryManager.saveUserMessage('What is the weather today?');
  await memoryManager.saveAssistantMessage('The weather is sunny and warm.');
...
runLocalExample().catch(console.error); 